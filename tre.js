...
class TransfereGovAutomator {
    ...
    async navegarParaLinkFinal() {
        try {
            const finalUrl = 'https://tre.discricionarias.transferegov.sistema.gov.br/voluntarias/execucao/ListarLicitacoes/ListarLicitacoes.do?destino=ListarLicitacoes';
...
app.post('/processar', async (req, res) => {
    ...
    const spinner = ora.default('Processando...').start();
    let { instrumento } = req.body;
    let transfereGov = new TransfereGovAutomator(spinner);

    let username = '24427284191';
    let password = 'Orzil@2024';

    try {
        await showMessage("Faça login no sistema", spinner);
        await transfereGov.login(username, password, 'https://tre.idp.transferegov.sistema.gov.br/idp/login');
